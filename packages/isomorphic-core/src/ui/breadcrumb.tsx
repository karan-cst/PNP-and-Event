...
const Breadcrumb = ({
  ...
  return (
    <div
      className={cn("inline-flex items-center gap-2.5 text-base", className)}
    >
          ...
          <>
            {React.cloneElement(child, {
              className: cn(
                ...
                "text-base text-gray-700 last:text-gray-500",
                disableCurrent && "last:pointer-events-none",
                // "text-gray-700 last:text-gray-500",
                // disableCurrent && "last:pointer-events-none",
              ),
            })}
            {index < numOfItems - 1 &&
              (separatorVariant === "default" ? (
                <span
                  className={cn("text-base text-gray-500", separatorClassName)}
                >
                  {separator}
                </span>
              ) : (
