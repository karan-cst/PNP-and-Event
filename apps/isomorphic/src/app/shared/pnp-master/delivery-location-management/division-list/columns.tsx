...
import { DeliveryLocationType } from './table';
import cn from '@core/utils/class-names';
import DateCell from '@core/ui/date-cell';
import PencilIcon from '@core/components/icons/pencil';
import { useModal } from '@/app/shared/modal-views/use-modal';
import { CreateDeliveryLocationModalView } from '../division-page-header';
import { getStatusBadge } from '@core/components/table-utils/get-status-badge';

const columnHelper = createColumnHelper<DeliveryLocationType>();

export const DeliveryLocationListColumns = [
  ...
  columnHelper.display({
    ...
    size: 150,
    header: 'Division',
    cell: ({ row }) => <Text className="text-sm">{row.original.division}</Text>,
  }),
  columnHelper.display({
    id: 'deliveryDays',
    size: 150,
    ...
    cell: ({ row }) => (
      <Text className="text-sm">{row.original?.deliveryDays.join(',')}</Text>
    ),
  }),
  ...
  columnHelper.display({
    ...
    cell: ({
      ...
      <Flex align="center" justify="start" gap="3" className="pe-4">
        <DeliveryLocationEdit deliveryLocation={row.original} />
    ...
    <Tooltip size="sm" content={'Edit Division'} placement="top" color="invert">
      <ActionIcon
        ...
        onClick={() =>
          openModal({
            view: (
              <CreateDeliveryLocationModalView
